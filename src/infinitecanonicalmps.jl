...
function finite_mps(ψ::InfiniteCanonicalMPS, range::AbstractRange)
  ...
  l̃N = sim(lN)
  δl0 = δ(Bool, dag(l̃0), l0)
  δlN = δ(Bool, dag(l̃N), lN)
  ψ_finite[1] *= δl0
  ψ_finite[N] *= dag(δlN)
...
function ITensors.expect(ψ::InfiniteCanonicalMPS, h::MPO)
  ...
  δˢ(n) = ITensorInfiniteMPS.δ(Bool, dag(s[n]), prime(s[n]))
  δˡ(n) = ITensorInfiniteMPS.δ(Bool, l[n], prime(dag(l[n])))
  δʳ(n) = ITensorInfiniteMPS.δ(Bool, dag(r[n]), prime(r[n]))
  ψ′ = prime(dag(ψ))
